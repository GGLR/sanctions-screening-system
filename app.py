...
# Paths - use streamlit config dir if available, otherwise use script location
try:
    import streamlit as st
    BASE_DIR = st.session_state.get('app_base_dir', os.path.dirname(os.path.abspath(__file__)))
except:
    BASE_DIR = os.path.dirname(os.path.abspath(__file__))

# For Streamlit Cloud, also check current working directory
if not os.path.exists(os.path.join(BASE_DIR, 'moha_sanctions_list.xml')):
    BASE_DIR = os.getcwd()

DB_PATH = os.path.join(BASE_DIR, "sanctions.db")
...
def init_database():
    """Initialize database schema - match local database schema"""
    conn = get_db_connection()
    cursor = conn.cursor()
    
    # Check if table already exists with correct schema
    cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='sanctions'")
    table_exists = cursor.fetchone()
    
    if table_exists:
        # Check if it has the old schema (normalized_name column)
        cursor.execute("PRAGMA table_info(sanctions)")
        columns = [row[1] for row in cursor.fetchall()]
        
        if 'normalized_name' in columns:
            # Table has old schema, need to recreate
    ...
    # Add id_number_2 and id_type_2 columns if not exists
    cursor.execute("PRAGMA table_info(sanctions)")
    columns = [row[1] for row in cursor.fetchall()]
...
def save_screening_log(branch: str, customer_name: str, result: Dict) -> bool:
    ...
    try:
        ...
        if result.get('matches'):
            # Get highest score from matches
            max_score = max(m['match_score'] for m in result['matches'])
            screening_score = max_score
        ...
        cursor.execute("""
            INSERT INTO screening_log (branch, customer_name, screening_score, match_count, risk_level, screened_at)
            VALUES (?, ?, ?, ?, ?, ?)
        """, (branch, customer_name, screening_score, match_count, risk_level, malaysia_time))
        
        conn.commit()
        conn.close()
        return True
    except Exception as e:
        print(f"Error saving screening log: {e}")
        return False

def get_screening_logs(limit: int = 100) -> List[Dict]:
    """Get screening logs from database"""
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM screening_log ORDER BY screened_at DESC LIMIT ?", (limit,))
...
def add_sanction(full_name: str, date_of_birth: str = None, nationality: str = None, 
                id_number: str = None, id_number_2: str = None, source: str = "MANUAL", list_type: str = "OTHER"):
    ...
    try:
        cursor.execute("""
            ...
            (full_name, normalized_name, date_of_birth, nationality, id_number, id_number_2, source, list_type)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """, (full_name.upper(), normalized, date_of_birth, nationality, id_number, id_number_2, source, list_type))
        conn.commit()
...
def add_bulk_sanctions(records: List[Dict]) -> int:
    ...
    for record in records:
        try:
            # Generate unique hash based on name, dob, nationality
            hash_input = f"{record.get('full_name', '')}{record.get('date_of_birth', '')}{record.get('nationality', '')}"
            import hashlib
            unique_hash = hashlib.md5(hash_input.encode()).hexdigest()
            
            cursor.execute("""
                INSERT OR IGNORE INTO sanctions 
                (unique_hash, full_name, date_of_birth, nationality, id_number, id_number_2, source, created_at, updated_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                unique_hash,
...
def auto_load_xml_files() -> Dict[str, int]:
    ...
    # Check if database already has data (from pre-loaded database)
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT COUNT(*) FROM sanctions")
    total_count = cursor.fetchone()[0]
    
    # Check if MOHA records need update (to get id_number_2)
    cursor.execute("SELECT COUNT(*) FROM sanctions WHERE source='MOHA'")
    moha_count = cursor.fetchone()[0]
    
    cursor.execute("SELECT COUNT(*) FROM sanctions WHERE source='MOHA' AND id_number_2 IS NULL")
    moha_needs_update = cursor.fetchone()[0] > 0
    conn.close()
    
    # If database has data and MOHA doesn't need update, skip auto-load
    if total_count > 0 and moha_count > 0 and not moha_needs_update:
        return loaded
    
    # Clear existing MOHA data if needs update or if no MOHA records
    if moha_needs_update or moha_count == 0:
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("DELETE FROM sanctions WHERE source='MOHA'")
        conn.commit()
        conn.close()
    ...
    try:
        ...
        for person in individuals:
            ...
            if third_name is not None and third_name.text:
                name_parts.append(third_name.text.strip())
...
def clean_dob(dob: str) -> str:
    ...
    # Replace dots, slashes, spaces with dashes for consistent parsing
    dob_normalized = dob.replace('.', '-').replace('/', '-').replace(' ', '-')
    
    # Try various date formats
    formats = ['%d-%m-%Y', '%d/%m/%Y', '%Y-%m-%d', '%d %B %Y', '%B %d, %Y', '%d-%m-%y', '%d.%m.%Y', '%d.%m.%y']
    
    for fmt in formats:
        try:
            dt = datetime.strptime(dob_normalized, fmt)
...
def calculate_id_match(query_id: str, sanction_id: str, sanction_id_2: str = None) -> bool:
    """Check if ID numbers match exactly (supports IC or Passport)"""
    if not query_id:
        return False
    ...
    # Check secondary ID (if exists)
    if sanction_id_2:
        s_id_2 = re.sub(r'[\s\-]', '', sanction_id_2.upper())
        if q_id == s_id_2:
            return True
...
def screen_name(full_name: str, date_of_birth: str = None, nationality: str = None, 
    ...
    for sanction in sanctions:
        ...
        id_match = calculate_id_match(id_number, sanction.get("id_number"), sanction.get("id_number_2"))
        
        # Calculate final score based on matching fields
        # Count matching fields
        matching_fields_count = sum([name_score > 0, dob_match, nat_match, id_match])
        
        has_dob_or_id = dob_match or id_match
        
        if name_score == 100.0:
            # Exact name match
            if has_dob_or_id and nat_match:
                # All major fields match - 100%
                final_score = 100.0
            elif has_dob_or_id:
                # Name + DOB or ID - 95%
                final_score = 95.0
            ...
            # Fuzzy name match
            if has_dob_or_id and nat_match:
                final_score = name_score * 0.95
            elif has_dob_or_id:
                final_score = name_score * 0.85
            elif nat_match:
                final_score = name_score * 0.75
            else:
            ...
            for entry in entries:
                ...
                for field in entry.findall(".//field"):
                    ...
                    if field_name == "(3) Name":
                        ...
                        passport_num = field_value
                    elif field_name == "(11) Identification Card Number":
                        ic_num = field_value
                
                # Store first ID as primary, second as secondary
                id_number = passport_num if passport_num else ic_num
                id_number_2 = ic_num if (passport_num and ic_num) else None
...
def show_match_alert(matches: List[Dict]):
    ...
    # Check for 100% matches (exact match = name_score 100% AND has DOB or ID match)
    ...
    # Check for HIGH risk (only if no exact matches)
    high_risk = [m for m in matches if m["risk_level"] == "HIGH"]
    if high_risk and not exact_matches:
...
def render_screening_form():
    ...
    # Branch selection
    branches = ["Midvalley", "Chowkit", "Klang", "Lot10", "Pavilion", "PBJ", "Summit"]
    ...
    if submitted and full_name:
        with st.spinner("Screening..."):
            ...
            # Save screening log
            save_screening_log(branch, full_name, result)
...
def render_screening_log():
    """Render screening log section"""
    st.header("📋 Screening Log")
    
    # Get screening logs
    logs = get_screening_logs(limit=100)
    
    if not logs:
        st.info("No screening records found. Perform some screenings to see logs here.")
        return
    
    # Stats
    total_screenings = len(logs)
    high_risk_count = sum(1 for log in logs if log.get('risk_level') == 'HIGH')
    medium_risk_count = sum(1 for log in logs if log.get('risk_level') == 'MEDIUM')
    matches_found = sum(1 for log in logs if log.get('match_count', 0) > 0)
    
    col1, col2, col3, col4 = st.columns(4)
    col1.metric("Total Screenings", total_screenings)
    col2.metric("High Risk", high_risk_count, delta_color="inverse")
    col3.metric("Medium Risk", medium_risk_count)
    col4.metric("Matches Found", matches_found)
    ...
    st.dataframe(
        ...
        column_config={
            ...
            "screening_score": st.column_config.NumberColumn("Score", format="%.1f%%"),
            "match_count": st.column_config.NumberColumn("Matches", format="%d"),
            "risk_level": st.column_config.TextColumn("Risk Level", width="small"),
            "screened_at": st.column_config.DatetimeColumn("Date (MYT)", format="DD/MM/YYYY HH:mm:ss"),
        },
...
def render_database_management():
    ...
    # Check if admin is logged in (use session state)
    if 'admin_logged_in' not in st.session_state:
        st.session_state['admin_logged_in'] = False
    
    # Password check for admin features
    if not st.session_state['admin_logged_in']:
        st.warning("🔒 Admin access required for database management")
        ...
        # Show basic stats only (no editing)
        stats = get_sanction_count()
    ...
    # Admin is logged in - show all features
    st.success("✅ Admin logged in - Full access granted")
    ...
    # Refresh from local XML files
    st.subheader("🔄 Refresh from Local Files")
    st.info("Reload sanctions data from local XML files (moha_sanctions_list.xml and un_sanctions_list.xml)")
    
    col_r1, col_r2 = st.columns(2)
    with col_r1:
        if st.button("Reload MOHA List", use_container_width=True):
            with st.spinner("Loading MOHA XML..."):
                # Delete existing MOHA records
                conn = get_db_connection()
                cursor = conn.cursor()
                cursor.execute("DELETE FROM sanctions WHERE source='MOHA'")
                conn.commit()
                ...
                if os.path.exists(MOHA_XML_FILE):
                    try:
                        ...
                        if records:
                            ...
                            st.success(f"✅ Reloaded {added} MOHA records")
                        else:
                            st.warning("No records found in MOHA XML")
                    except Exception as e:
                        st.error(f"Error: {e}")
                else:
                    st.error(f"MOHA XML file not found: {MOHA_XML_FILE}")
                st.rerun()
    
    with col_r2:
        if st.button("Reload UN List", use_container_width=True):
            with st.spinner("Loading UN XML..."):
                # Delete existing UN records
                conn = get_db_connection()
                cursor = conn.cursor()
                cursor.execute("DELETE FROM sanctions WHERE source='UN'")
                conn.commit()
                conn.close()
                ...
                if os.path.exists(UN_XML_FILE):
                    try:
                        ...
                        if records:
                            added = add_bulk_sanctions(records)
                            st.success(f"✅ Reloaded {added} UN records")
                        else:
                            st.warning("No records found in UN XML")
                    except Exception as e:
                        st.error(f"Error: {e}")
                else:
                    st.error(f"UN XML file not found: {UN_XML_FILE}")
                st.rerun()
    ...
    with tab1:
        xml_type = st.radio("XML Type", ["MOHA (Malaysia)", "UN (United Nations)"], horizontal=True)
            ...
            if st.button("Process XML File"):
                try:
                    ...
                    if records:
                        ...
                        conn = get_db_connection()
                        cursor = conn.cursor()
                        cursor.execute("DELETE FROM sanctions WHERE source=?", (source,))
                        conn.commit()
                        conn.close()
                        
                        # Add new records
                        added = add_bulk_sanctions(records)
                        st.success(f"✅ Added {added} {source} records")
                        time.sleep(1)
                        st.rerun()
                    else:
...
def main():
    ...
    page = st.sidebar.radio("Navigate", ["Screening", "Screening Log", "Database", "Settings"])
    
    if page == "Screening":
        render_screening_form()
    elif page == "Screening Log":
        render_screening_log()
    elif page == "Database":
