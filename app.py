        ...
        for sanction in sanctions:
            ...
            if name_score >= threshold:
                ...
                if dob and sanction.get('date_of_birth'):
                    ...
                    # Try various date formats (3.6.1966, 03.06.1966, 1966-06-03, etc.)
                    user_parts = dob.replace('-', '/').replace('.', ' ').split()
                    db_parts = db_dob.replace('-', '/').replace('.', ' ').split()
                    
                    # Normalize: remove leading zeros and compare
                    user_normalized = ''.join(p.lstrip('0') for p in user_parts if p)
                    db_normalized = ''.join(p.lstrip('0') for p in db_parts if p)
                    
                    if user_normalized and db_normalized and (user_normalized in db_normalized or db_normalized in user_normalized):
                        dob_match = True
...
def screening_log_page():
    ...
    if logs:
        ...
        # Format the datetime - already stored in Malaysia time (UTC+8), display as-is
