...
def parse_un_xml(xml_content: str) -> List[Dict]:
    ...
    try:
        ...
        # Try different XML structures for UN list
        for person in root.findall(".//INDIVIDUAL") + root.findall(".//Individual") + root.findall(".//PERSON") + root.findall(".//record"):
            name = None
            dob = None
            nationality = None
            id_number = None
            
            # Try various tag names for name
            for name_tag in ["FIRST_NAME", "FIRSTNAME", "Title", "COMMENTS1"]:
            ...
            for name_tag in ["FULL_NAME", "FullName", "NAME"]:
                name_elem = person.find(name_tag)
                if name_elem is not None and name_elem.text:
                    name = name_elem.text
                    break
            
            # If no structured name, get from DESCR element
            if not name:
                descr = person.find("DESCR")
                if descr is not None and descr.text:
                    name = descr.text
            
            # Get DOB
            for dob_tag in ["DOB", "DateOfBirth", "DATE_OF_BIRTH"]:
            ...
            # Get nationality
            for nat_tag in ["NATIONALITY", "Nationality", "Country"]:
            ...
            # Get ID
            for id_tag in ["ID", "ID_NUMBER", "DocID", "NUMBER"]:
...
def parse_moha_xml(xml_content: str) -> List[Dict]:
    ...
    try:
        ...
        st.error(f"Error parsing MOHA XML: {e}")
