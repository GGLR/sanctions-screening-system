...
def init_database():
    ...
    # Add id_number_2 column if not exists
    cursor.execute("PRAGMA table_info(sanctions)")
    columns = [row[1] for row in cursor.fetchall()]
...
def auto_load_xml_files() -> Dict[str, int]:
    ...
    if count > 0 and not moha_needs_update:
        return loaded
    
    # Clear existing MOHA data if needs update
    if moha_needs_update:
        conn = get_db_connection()
        cursor = conn.cursor()
    ...
    # Try to load MOHA XML only if database is empty
