        ...
        # Refresh from local files
            ...
            try:
                ...
                for source, filepath in config.LOCAL_XML_FILES.items():
                    if filepath.exists():
                        try:
                            records = parse_local_file(str(filepath), source)
                ...
                for source, filepath in config.LOCAL_XML_FILES.items():
                    if filepath.exists():
                        try:
                            ...
                            for elem in root:
                                record = {
                                    'full_name': elem.findtext('.//Name') or elem.findtext('.//FULL_NAME') or elem.findtext('.//name') or '',
                                    'date_of_birth': elem.findtext('.//DateOfBirth') or elem.findtext('.//DOB') or elem.findtext('.//dob') or '',
                                    'nationality': elem.findtext('.//Nationality') or elem.findtext('.//nationality') or '',
                                    'id_number': elem.findtext('.//IdNumber') or elem.findtext('.//ID') or elem.findtext('.//id_number') or '',
                                    'source': source
                                }
                                if record['full_name']:
