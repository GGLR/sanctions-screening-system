...
def auto_init_database():
    """
    Auto-initialize the database from local XML files on first startup.
    This ensures the database is populated on Streamlit Cloud where the
    SQLite file is ephemeral and reset on every app restart/wake.
    Runs once per app session (cached by st.cache_resource).
    """
    import sqlite3
    ...
    db_path = None
    for p in ["data/sanctions.db", "sanctions.db"]:
        if os.path.exists(p):
            db_path = p
            break
    if db_path is None:
    ...
    ''')
    cursor.execute('CREATE INDEX IF NOT EXISTS idx_name ON sanctions(full_name)')
    cursor.execute('CREATE INDEX IF NOT EXISTS idx_source ON sanctions(source)')
    conn.commit()

    # Check if already populated with all 3 sources
    cursor.execute("SELECT source, COUNT(*) FROM sanctions WHERE is_active=1 GROUP BY source")
    existing = {row[0]: row[1] for row in cursor.fetchall()}
    conn.close()

    sources_needed = []
    if existing.get("MOHA_MALAYSIA", 0) == 0:
        sources_needed.append(("moha_sanctions_list.xml", "MOHA_MALAYSIA", "MOHA"))
    if existing.get("UN_LIST", 0) == 0:
        sources_needed.append(("un_sanctions_list.xml", "UN_LIST", "UN"))
    if existing.get("PEP_LIST", 0) == 0:
        sources_needed.append(("pep_list.xml", "PEP_LIST", "PEP"))
    ...
    try:
        ...
        db = SanctionsDatabase(db_path)

        for xml_file, source_label, parser_type in sources_needed:
            # Search for the XML file
            xml_path = None
            for base in [".", os.path.dirname(os.path.abspath(__file__))]:
                candidate = os.path.join(base, xml_file)
            ...
            try:
                records = parse_local_file(xml_path, source_type=parser_type)
                if records:
                    db.add_sanctions_batch(records, source=source_label)
            except Exception:
                pass

    except Exception:
        # If xml_parser not available, use inline parsing for MOHA and UN
        conn = sqlite3.connect(db_path)
        cursor = conn.cursor()
                ...
                if parser_type == "MOHA":
                    ...
                    for entry in root.findall('.//entry'):
                        ...
                        for field in entry.findall('field'):
                            n = field.get('name', '')
                            v = field.text or ''
                            if 'Name' in n and not full_name:
                                full_name = v
                            elif 'Date of Birth' in n and not dob:
                                dob = v
                            elif 'Nationality' in n and not nationality:
                                nationality = v
                            elif ('Passport' in n or 'ID' in n) and not id_number:
                                id_number = v
                        if full_name:
                            h = hashlib.md5(f'{full_name}|{dob}|{id_number}'.encode()).hexdigest()
                            try:
                                cursor.execute('INSERT OR IGNORE INTO sanctions (unique_hash,full_name,date_of_birth,nationality,id_number,source,created_at,updated_at,is_active) VALUES (?,?,?,?,?,?,?,?,1)',
                                               (h, full_name, dob, nationality, id_number, source_label, now, now))
                            except Exception:
                                pass
                    ...
                    for entry in root.findall('.//INDIVIDUAL'):
                        parts = []
                        for tag in ['FIRST_NAME', 'SECOND_NAME', 'THIRD_NAME', 'LAST_NAME']:
                            el = entry.find(f'.//{tag}')
                        ...
                        if not full_name:
                            continue
                        dob_el = entry.find('.//DATE_OF_BIRTH')
                        dob = dob_el.text if dob_el is not None and dob_el.text else ''
                        nat_el = entry.find('.//NATIONALITY')
                        nationality = nat_el.text if nat_el is not None and nat_el.text else ''
                        id_el = entry.find('.//NUMBER')
                        id_number = id_el.text if id_el is not None and id_el.text else ''
                        h = hashlib.md5(f'{full_name}|{dob}|{id_number}'.encode()).hexdigest()
                        try:
                            cursor.execute('INSERT OR IGNORE INTO sanctions (unique_hash,full_name,date_of_birth,nationality,id_number,source,created_at,updated_at,is_active) VALUES (?,?,?,?,?,?,?,?,1)',
                                           (h, full_name, dob, nationality, id_number, source_label, now, now))
                    ...
                    for person in root.findall('.//Person'):
                        ...
                        dob = dob_el.text.strip() if dob_el is not None and dob_el.text else ''
                        nat_el = person.find('Nationality')
                        nationality = nat_el.text.strip().upper() if nat_el is not None and nat_el.text else ''
                        if nationality == 'UNKNOWN':
                            nationality = ''
                        h = hashlib.md5(f'{full_name}|{dob}|'.encode()).hexdigest()
                        try:
                            cursor.execute('INSERT OR IGNORE INTO sanctions (unique_hash,full_name,date_of_birth,nationality,id_number,source,created_at,updated_at,is_active) VALUES (?,?,?,?,?,?,?,?,1)',
                                           (h, full_name, dob, nationality, None, source_label, now, now))
                        except Exception:
                            pass
...
def _get_malaysia_now():
    """Return current Malaysia time (UTC+8) as formatted string."""
    from datetime import datetime, timezone, timedelta
    malaysia_tz = timezone(timedelta(hours=8))
    return datetime.now(malaysia_tz).strftime('%Y-%m-%d %H:%M:%S')


def save_screening_log(branch: str, customer_name: str, total_matches: int, high_risk: int, medium_risk: int, low_risk: int, risk_level: str, exact_match: bool = False, source: str = ""):
    ...
    try:
        ...
        try:
            cursor.execute('ALTER TABLE screening_log ADD COLUMN source TEXT')
        except:
            pass

        # Use Malaysia time (UTC+8)
        now = _get_malaysia_now()
        cursor.execute('''
            INSERT INTO screening_log (branch, customer_name, total_matches, high_risk, medium_risk, low_risk, risk_level, exact_match, source, screened_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', (branch, customer_name, total_matches, high_risk, medium_risk, low_risk,
              risk_level, 1 if exact_match else 0, source, now))
...
def get_screening_log(limit: int = 10000):
    ...
    if endpoint == "/api/screen" and method == "POST":
        ...
        for sanction in sanctions:
            ...
            if name_score >= threshold:
                ...
                # Check DOB - use matching_engine parser for consistent format handling
                if dob and sanction.get('date_of_birth'):
                    db_dob = sanction.get('date_of_birth', '')
                    try:
                        from matching_engine import FuzzyMatchingEngine
                        _dob_match, _dob_score = FuzzyMatchingEngine._calculate_dob_score(
                            FuzzyMatchingEngine, dob, db_dob
                        )
                        if _dob_match:
                            dob_match = True
                        ...
                        def _norm_dob(d):
                            ...
                            if m2:
                                return d.replace('/', '-')
                            return d
                        if _norm_dob(dob) == _norm_dob(db_dob):
                            dob_match = True
                            matched_fields.append("date of birth")
                ...
                exact_4 = dob_match and nationality_match and id_match and name_score < 100  # DOB + nationality + ID (no 100% name)
                exact_5 = dob_match and id_match and name_score < 100  # DOB + ID (no 100% name)
    ...
    if endpoint == "/api/refresh/local" and method == "POST":
        ...
        for source, (filepath, source_name) in found.items():
            try:
                ...
                # Try UN format
                for entry in root.findall('.//INDIVIDUAL'):
                    # Parse all name parts: FIRST_NAME, SECOND_NAME, THIRD_NAME, LAST_NAME
                    ...
                    for name_part in [first, second, third, last]:
                    ...
                    # Parse DOB - check DATE first (format: 1966-06-03), then YEAR/MONTH/DAY
                    dob = ''
                    dob_date = entry.find('.//INDIVIDUAL_DATE_OF_BIRTH/DATE')
                    if dob_date is not None and dob_date.text:
                        # Convert 1966-06-03 to 03.06.1966
                        parts = dob_date.text.split('-')
                        if len(parts) == 3:
                            dob = f'{parts[2]}.{parts[1]}.{parts[0]}'
                    else:
                        # Try YEAR, MONTH, DAY separately
                        dob_year = entry.find('.//INDIVIDUAL_DATE_OF_BIRTH/YEAR')
                        dob_month = entry.find('.//INDIVIDUAL_DATE_OF_BIRTH/MONTH')
                        dob_day = entry.find('.//INDIVIDUAL_DATE_OF_BIRTH/DAY')
                        if dob_year is not None and dob_year.text:
                            dob = dob_year.text
                        if dob_month is not None and dob_month.text:
                            dob = (dob_month.text + '.') + dob
                        if dob_day is not None and dob_day.text:
                            dob = dob_day.text + '.' + dob
                    
                    # Parse Nationality - can have multiple VALUES inside NATIONALITY
                    nationality = ''
                    nat_elem = entry.find('.//NATIONALITY')
                    if nat_elem is not None:
                        nat_values = nat_elem.findall('.//VALUE')
                        nationalities = [v.text for v in nat_values if v.text]
                        nationality = ', '.join(nationalities)
                    
                    # Parse ID/Document - check INDIVIDUAL_DOCUMENT
                    id_elem = entry.find('.//INDIVIDUAL_DOCUMENT/NUMBER')
                ...
                for record in records:
                    ...
                    try:
                        cursor.execute('''
                            INSERT INTO sanctions (unique_hash, full_name, date_of_birth, nationality, id_number, source, comments, created_at, updated_at, is_active)
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 1)
                        ''', (unique_hash, name, dob, record.get('nationality'), id_num, source_name, comments, now, now))
                        added += 1
                    except:
                        skipped += 1
...
def main():
    ...
    # Auto-initialize database from XML files on startup (handles Streamlit Cloud ephemeral filesystem)
    auto_init_database()
    ...
    /* Hide Streamlit branding: hamburger menu, footer, and header deploy/GitHub button */
    ...
    with st.sidebar:
        ...
        if stats:
            ...
            pep_count = by_source.get("PEP List", 0)
...
def screening_page():
    ...
    if submitted and full_name:
        ...
        if result:
            # Determine the source of the match (UN List or MOHA)
            match_source = ""
            if result.get("matches"):
                # Get the source from the first (highest scoring) match
            ...
            with st.expander(f"{match['full_name']} - {match['risk_level']} Risk (Score: {match['match_score']}%)", 
                ...
                with col2:
                    ...
                    if match.get('dob_match'):
                        st.write(f"✅ DOB Match: {match.get('matched_dob', 'NA')}")
                    else:
                        db_dob = match.get('matched_dob') or 'NA'
                        st.write(f"❌ DOB: {db_dob}")
                    
                    if match.get('nationality_match'):
                        st.write(f"✅ Nationality Match: {match.get('matched_nationality', 'NA')}")
                    else:
                        db_nat = match.get('matched_nationality') or 'NA'
                        st.write(f"❌ Nationality: {db_nat}")
                    
                    if match.get('id_match'):
                        st.write(f"✅ ID Match: {match.get('matched_id', 'NA')}")
                    else:
                        db_id = match.get('matched_id') or 'NA'
                        st.write(f"❌ ID: {db_id}")
    else:
...
def screening_log_page():
    ...
    # Get screening log (no limit - show all records)
    logs = get_screening_log()
    ...
    if logs:
        ...
        # Format the datetime - handle mixed formats (ISO with T, space-separated, etc.)
        def _fmt_dt(val):
            if not val:
                return ''
            try:
                import pandas as _pd
                return _pd.to_datetime(str(val)).strftime('%Y-%m-%d %H:%M:%S')
            except Exception:
                return str(val)[:19].replace('T', ' ')
        df['screened_at'] = df['screened_at'].apply(_fmt_dt)
        
        # Date filter
        malaysia_tz = timezone(timedelta(hours=8))
        today_my = datetime.now(malaysia_tz).date()
        
        col_filter1, col_filter2 = st.columns([2, 2])
        with col_filter1:
            filter_date = st.date_input("Filter by Date (Malaysia Time)", value=today_my)
        with col_filter2:
            show_all = st.checkbox("Show All Dates", value=False)
        
        if not show_all:
            df = df[df['screened_at'].str.startswith(str(filter_date))]
        
        st.caption(f"⏰ All times shown in Malaysia Time (UTC+8). Showing {len(df)} record(s).")
        
        # Format exact match column
        if 'exact_match' in df.columns:
            df['100% Match'] = df['exact_match'].apply(lambda x: '✅ YES' if x == 1 else '❌ NO')
        
        # Format source column - rename UN_LIST to UN List, MOHA_MALAYSIA to MOHA List, PEP_LIST to PEP List
        if 'source' in df.columns:
            df['source'] = df['source'].replace({'UN_LIST': 'UN List', 'MOHA_MALAYSIA': 'MOHA List', 'PEP_LIST': 'PEP List'})
        ...
        col1, col2, col3, col4 = st.columns(4)
        with col1:
            st.metric("Total Screenings", len(filtered_logs))
        with col2:
            exact_count = sum(1 for l in filtered_logs if l.get('exact_match') == 1)
            st.metric("100% Match", exact_count)
        with col3:
            high_count = sum(1 for l in filtered_logs if l.get('risk_level') == 'HIGH')
            st.metric("High Risk", high_count)
        with col4:
            low_risk_count = sum(1 for l in filtered_logs if l.get('risk_level') == 'LOW')
