...
def save_screening_log(branch: str, customer_name: str, total_matches: int, high_risk: int, medium_risk: int, low_risk: int, risk_level: str, exact_match: bool = False, source: str = ""):
    ...
    try:
        ...
        # Try to add source column if not exists
        try:
        ...
        cursor.execute('''
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', (branch, customer_name, total_matches, high_risk, medium_risk, low_risk, risk_level, 1 if exact_match else 0, source, now))
...
def screening_log_page():
    ...
    if logs:
        ...
        # Format source column - rename UN_LIST to UN List, MOHA_MALAYSIA to MOHA List
        if 'source' in df.columns:
            df['source'] = df['source'].replace({'UN_LIST': 'UN List', 'MOHA_MALAYSIA': 'MOHA List'})
