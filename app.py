...
def save_screening_log(branch: str, customer_name: str, total_matches: int, high_risk: int, medium_risk: int, low_risk: int, risk_level: str):
    ...
    try:
        ...
        cursor.execute('''
            INSERT INTO screening_log (branch, customer_name, total_matches, high_risk, medium_risk, low_risk, risk_level, screened_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        ''', (branch, customer_name, total_matches, high_risk, medium_risk, low_risk, risk_level, now))
    ...
    if endpoint == "/api/screen" and method == "POST":
        ...
        for sanction in sanctions:
            ...
            if name_score >= threshold:
                ...
                # Determine risk level based on name score AND additional matches
                # HIGH risk: 100% name + DOB (mandatory), OR 100% name + ID number
                if name_score == 100 and (dob_match or id_match):
                    risk_level = "HIGH"
                # MEDIUM risk: 100% name + nationality (without DOB or ID), OR 85%+ name + any other field
                elif name_score == 100 and nationality_match:
                    risk_level = "MEDIUM"
                elif name_score >= 85:
                    risk_level = "HIGH"
                elif name_score >= 70:
                ...
                # Exact match = 100% name + DOB (mandatory) OR 100% name + ID
                is_exact = (name_score == 100 and dob_match) or (name_score == 100 and id_match)
...
def screening_log_page():
    ...
    if not config.PASSWORD_PROTECTION_ENABLED:
        show_log = True
    elif 'admin_authenticated' in st.session_state and st.session_state.admin_authenticated:
        show_log = True
    else:
        show_log = False
        show_login_form()
        ...
        if logs:
            ...
            with col3:
                medium_count = sum(1 for l in logs if l.get('risk_level') == 'MEDIUM')
                st.metric("Medium Risk", medium_count)
            with col4:
                low_count = sum(1 for l in logs if l.get('risk_level') == 'LOW')
