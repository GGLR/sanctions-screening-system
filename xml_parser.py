...
class PEPParser(XMLParser):
    """
    Parser for PEP (Politically Exposed Persons) list
    Format: <PoliticallyExposedPersons> root with <Person> entries
    Source: https://politikus.sinarproject.org
    """
    ...
    def parse(self, xml_content: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # PEP list uses <Person> tags under <PoliticallyExposedPersons>
            persons = root.findall('.//Person') + root.findall('.//PERSON')
            ...
            logger.info(f"Parsed {len(records)} records from PEP XML")

        except Exception as e:
            logger.error(f"Error parsing PEP XML: {e}")
            raise

        return records

    def _parse_person(self, element: ET.Element) -> Optional[Dict[str, Any]]:
        """Parse a single PEP person entry"""
        record = {
            'name': None,
            'dob': None,
            'nationality': None,
        ...
        # Get name - use explicit is not None checks (ET elements are falsy when childless)
        name_el = element.find('Name')
        if name_el is None:
            name_el = element.find('NAME')
        if name_el is not None and name_el.text:
            record['name'] = self._clean_text(name_el.text)

        # Get date of birth
        dob_el = element.find('DateOfBirth')
        if dob_el is None:
            dob_el = element.find('DATE_OF_BIRTH')
        if dob_el is None:
            dob_el = element.find('DOB')
        if dob_el is not None and dob_el.text:
            record['dob'] = self._extract_date(dob_el.text)

        # Get nationality
        nat_el = element.find('Nationality')
        if nat_el is None:
            nat_el = element.find('NATIONALITY')
        if nat_el is not None and nat_el.text:
            nat_text = self._clean_text(nat_el.text).upper()
            if nat_text and nat_text != 'UNKNOWN':
                record['nationality'] = nat_text

        # Build comments from Position, Organization, and SourceURL
        comments_parts = []
        position_el = element.find('Position')
        if position_el is None:
            position_el = element.find('POSITION')
        if position_el is not None and position_el.text:
            pos_text = self._clean_text(position_el.text)
            if pos_text and pos_text.upper() != 'UNKNOWN':
                comments_parts.append(f"Position: {pos_text}")

        org_el = element.find('Organization')
        if org_el is None:
            org_el = element.find('ORGANIZATION')
        if org_el is not None and org_el.text:
            org_text = self._clean_text(org_el.text)
            if org_text and org_text.upper() not in ('UNKNOWN', 'ON BEHALF OF'):
                comments_parts.append(f"Organization: {org_text}")

        source_url_el = element.find('SourceURL')
        if source_url_el is None:
            source_url_el = element.find('SOURCEURL')
        if source_url_el is not None and source_url_el.text:
            comments_parts.append(f"Source: {self._clean_text(source_url_el.text)}")

        if comments_parts:
            record['comments'] = ' | '.join(comments_parts)
