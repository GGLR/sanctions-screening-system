...
def check_api_connection():
    ...
    try:
        db = get_db()
        # Check if get_statistics method exists
        if hasattr(db, 'get_statistics'):
            stats = db.get_statistics()
            return stats is not None
        else:
            st.warning("Database method get_statistics not found")
            return False
    except Exception as e:
...
def call_api(endpoint: str, method: str = "GET", data: dict = None, files: dict = None):
    ...
    try:
        ...
        # Get official statistics (filtered)
            ...
            stats = db.get_statistics()
            # Filter to only show UN and MOHA
            by_source = stats.get("by_source", {})
            filtered = {k: v for k, v in by_source.items() if k in ["UN Sanction List", "MOHA List"]}
