...
class FuzzyMatchingEngine:
    ...
    def _parse_dob(dob: str) -> Optional[str]:
        ...
        Returns ISO format 'YYYY-MM-DD' when full date is available,
        or 'YYYY' when only year is available.
        Supports: DD/MM/YYYY, YYYY-MM-DD, DD-MM-YYYY, DD.MM.YYYY, YYYY, etc.
        """
        if not dob:
            return None
        ...
        # Try full date with regex: DD/MM/YYYY, DD-MM-YYYY, DD.MM.YYYY
        ...
        for num in numbers:
            if len(num) == 4 and 1900 <= int(num) <= 2010:
                return num
    ...
    def _calculate_dob_score(self, query_dob: str, target_dob: str) -> Tuple[bool, float]:
        ...
        Supports full date comparison (YYYY-MM-DD) and year-only fallback.
        Handles input formats: DD/MM/YYYY, YYYY-MM-DD, YYYY, etc.
        """
        query_parsed = self._parse_dob(query_dob)
        target_parsed = self._parse_dob(target_dob)

        if not query_parsed or not target_parsed:
            return False, 0.0

        # Both are full dates (YYYY-MM-DD)
        if len(query_parsed) == 10 and len(target_parsed) == 10:
            if query_parsed == target_parsed:
                return True, 100.0
            # Same year and month but different day (possible data entry error)
            if query_parsed[:7] == target_parsed[:7]:
                return True, 90.0
            # Same year only
            if query_parsed[:4] == target_parsed[:4]:
                return True, 70.0
            # Off by 1 year
            try:
                diff = abs(int(query_parsed[:4]) - int(target_parsed[:4]))
                if diff <= 1:
                    return True, 60.0
        ...
        query_year = query_parsed[:4]
        target_year = target_parsed[:4]
    ...
    def screen_customer(self, full_name: str, dob: str = None, 
        ...
        for sanction in all_sanctions:
            ...
            # New scoring logic per user requirements:
            # - Name matched = 40%
            # - Name + nationality matched = 50%
            # - Name + DOB matched = 100%
            # - Name + ID matched = 100%
            # - Name + ID + nationality + DOB = 100%
            
            has_name_match = name_score >= self.threshold
            has_dob = bool(dob and dob.strip())
            has_id = bool(id_number and id_number.strip())
            has_nationality = bool(nationality and nationality.strip())
            
            # Check what user provided and what matches
            dob_matches = dob_match and has_dob
            id_matches = id_score > 0 and has_id
            nationality_matches = nationality_match and has_nationality
            
            if dob_matches or id_matches:
                # Name + DOB OR Name + ID = 100%
                final_score = 100.0
            elif nationality_matches and has_name_match:
                # Name + nationality = 50%
                final_score = 50.0
            else:
                # Name only = 40%
