...
def call_api(endpoint: str, method: str = "GET", data: dict = None, files: dict = None):
    ...
    try:
        ...
        # Get official statistics (show all sources for debugging)
            ...
            # Debug: Show database location and contents
            dbg_paths = [
                "sanctions.db",
                "data/sanctions.db",
                "/mount/src/sanctions-screening-system/sanctions.db",
            ...
            for dp in dbg_paths:
                if os.path.exists(dp):
                    ...
                    cursor.execute('SELECT COUNT(*) FROM sanctions')
                    count = cursor.fetchone()[0]
                    st.write(f"Path {dp}: exists=True, records={count}")
                    conn.close()
                else:
                    st.write(f"Path {dp}: exists=False")
        ...
        # Refresh from local files
            ...
            search_paths = [
                ...
                "./",
                os.getcwd(),
                "/mount/src/sanctions-screening-system",
                "/mount/src",
            ...
            st.write("Debug - XML files found:", found_files)
            
            if not found_files:
                return {"success": False, "message": "No XML files found"}
            ...
            try:
                ...
                for key, (filepath, source) in found_files.items():
                    try:
                        ...
                        records = parse_local_file(fixed_path, source)
                        st.write(f"Debug - xml_parser returned {len(records)} records")
                        result = db.add_sanctions_batch(records, source)
                        results[source] = result
                    except Exception as e:
                        st.write(f"Debug - xml_parser error: {str(e)}")
                ...
                for key, (filepath, source) in found_files.items():
                    try:
                        st.write(f"Debug - Parsing {filepath}...")
                        ...
                        st.write(f"Debug - Found {len(entries)} MOHA entries")
                        ...
                        # Handle UN format
                        un_entries = root.findall('.//INDIVIDUAL')
                        st.write(f"Debug - Found {len(un_entries)} UN entries")
                        ...
                        st.write(f"Debug - Total records parsed: {len(records)}")
                        result = db.add_sanctions_batch(records, source)
                        st.write(f"Debug - Add result: {result}")
                        results[source] = result
                    except Exception as e:
